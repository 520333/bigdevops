...
func GetWithHeaders(logger *zap.Logger, funcName string, tw int, url string, paramsMap map[string]string, headersMap map[string]string, skipVerify ...bool) ([]byte, error) {
	client := &http.Client{}
	...
	if err != nil {
		logger.Error(fmt.Sprintf("[GetWithHeaders.http.NewRequest.error][funcName:%+v][url:%v][err:%v]", funcName, url, err))
		return nil, err
	}
	...
	resp, err := client.Do(req)
	if err != nil {
		logger.Error(fmt.Sprintf("[GetWithHeaders.request.error][funcName:%+v][url:%v][err:%v]", funcName, url, err))
		return nil, err
	}

	if resp.StatusCode != http.StatusOK {
		bodyBytes, _ := ioutil.ReadAll(resp.Body)
		logger.Error(fmt.Sprintf("[GetWithHeaders.StatusCode.not2xx.error][funcName:%+v][url:%v][code:%v][resp_body_text:%v][err:%v]", funcName, url, resp.StatusCode, string(bodyBytes), err))
		return nil, errors.New(fmt.Sprintf("server returned HTTP status %s", resp.Status))
	}
	defer func() {
		io.Copy(ioutil.Discard, resp.Body)
	...
	bodyBytes, err := ioutil.ReadAll(resp.Body)
	if err != nil {
		logger.Error(fmt.Sprintf("[GetWithHeaders.readbody.error][funcName:%+v][url:%v][err:%v]", funcName, url, err))
		return nil, err
	}
	return bodyBytes, err
}
